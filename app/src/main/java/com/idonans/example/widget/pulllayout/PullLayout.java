package com.idonans.example.widget.pulllayout;
...
public class PullLayout extends FrameLayout implements NestedScrollingParent2, NestedScrollingChild2 {
    ...
    private void initFromAttributes(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        mOffsetHelper = new OffsetHelper((offsetHelper, animating) -> {
            ...
            if (mTarget == null || mHeader == null) {
                ExampleLog.e("target or header not found");
                return;
            }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mActivePointerId < 0) {
                    ExampleLog.e("onInterceptTouchEvent ACTION_MOVE but no active pointer id.");
                    return false;
                }

                pointerIndex = event.findPointerIndex(mActivePointerId);
                if (pointerIndex < 0) {
                    ExampleLog.e("onInterceptTouchEvent ACTION_MOVE but active pointer id invalid.");
                    return false;
                }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mActivePointerId < 0) {
                    ExampleLog.e("onTouchEvent ACTION_MOVE but no active pointer id.");
                    return false;
                }

                pointerIndex = event.findPointerIndex(mActivePointerId);
                if (pointerIndex < 0) {
                    ExampleLog.e("onTouchEvent ACTION_MOVE but active pointer id invalid.");
                    return false;
                }
